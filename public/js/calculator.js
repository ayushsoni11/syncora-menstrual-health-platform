...
document.getElementById("calculate-button").addEventListener("click", function () {
    ...
    const cycleLengthInput = parseInt(document.getElementById("cycle-length").value, 10);
    const periodDurationInput = parseInt(document.getElementById("period-duration").value, 10);
    const bodyTempInput = parseFloat(document.getElementById("body-temp").value); // NEW
    const weightInput = parseFloat(document.getElementById("weight").value); // NEW

    if (!startDateInput || !cycleLengthInput || !periodDurationInput || !bodyTempInput || !weightInput) {
        alert("Please fill in all fields.");
        return;
    }
    ...
    // Pass the new inputs for future improvements
    showCalendars(startDate, cyclesLength, lastPeriodLength, bodyTempInput, weightInput);
});

function showCalendars(firstDay, cyclesLength, lastPeriodLength, bodyTemp, weight) {
    for (let i = 0; i < 3; i++) {
        const currentMonth = new Date(firstDay);
        currentMonth.setMonth(firstDay.getMonth() + i);
        ...
        console.log(`Body Temperature: ${bodyTemp}, Weight: ${weight}`);
    }
}
